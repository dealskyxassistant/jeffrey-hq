...
function AgentCard({
  ...
  const statusConfig = {
    active: { label: "Active", className: "bg-green-900/40 text-green-400 border-green-800", dot: "bg-green-400" },
    running: { label: "Running", className: "bg-indigo-900/40 text-indigo-400 border-indigo-800", dot: "bg-indigo-400 animate-pulse" },
    idle: { label: "Idle", className: "bg-yellow-900/40 text-yellow-400 border-yellow-800", dot: "bg-yellow-400" },
  };
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 hover:border-gray-700 transition-colors flex flex-col gap-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 bg-gray-800 rounded-lg flex items-center justify-center text-xl">
            {agent.emoji}
          </div>
          <div>
      ...
      <div className="flex flex-wrap gap-1">
        {agent.skills.slice(0, 3).map((s) => (
          <span key={s} className="text-[10px] text-gray-500 bg-gray-800 px-1.5 py-0.5 rounded">
            {s}
          </span>
        ))}
      </div>
      ...
      {lastResult && (
        <div className="bg-gray-800/50 rounded-lg p-2.5 text-xs">
          <div className="text-gray-500 mb-1">Letzter Output · {relativeTime(lastResult.createdAt)}</div>
          <div className="text-gray-300 line-clamp-2">{lastResult.content.split("\n")[0]}</div>
        </div>
      )}
      ...
      {lastJob && (
        <div className="text-xs text-gray-500">
          <span className="text-gray-600">Letzter Job: </span>
          <span className="text-gray-400 truncate">{lastJob.name}</span>
          {lastJob.lastRunAt && (
            <span className="text-gray-600"> · {relativeTime(lastJob.lastRunAt)}</span>
          )}
        </div>
      )}
      ...
      <div className="flex gap-2 mt-auto pt-1">
        <button
          ...
          className="flex-1 text-xs text-indigo-400 hover:text-indigo-300 bg-indigo-900/20 hover:bg-indigo-900/40 border border-indigo-800/40 px-3 py-1.5 rounded-lg transition-all text-center"
        >
          ▶ Run Job
        </button>
        <Link
          href={`/inbox?agentId=${agent.id}`}
          className="flex-1 text-xs text-gray-400 hover:text-gray-200 bg-gray-800 hover:bg-gray-700 px-3 py-1.5 rounded-lg transition-all text-center"
        >
          📬 Inbox
        </Link>
      </div>
...
export default function DashboardPage() {
  ...
  function getLiveData(agentId: string): AgentLiveData {
    ...
    const lastJob = agentJobs.sort((a, b) =>
        ...
        <div className="mb-8 flex items-start justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm">
              ...
              {!loading && (
                <span className="ml-2 text-xs text-gray-600">· Auto-refresh alle 30s</span>
              )}
            </p>
          </div>
          {newResultsCount > 0 && (
            <Link
              href="/inbox"
              className="flex items-center gap-2 bg-blue-900/30 border border-blue-800/40 text-blue-400 hover:text-blue-300 px-3 py-2 rounded-lg text-sm transition-colors"
            >
