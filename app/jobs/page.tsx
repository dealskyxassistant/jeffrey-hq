...
const STATUS_CONFIG: Record<JobStatus, { label: string; className: string; dot: string }> = {
  ...
  running: {
    label: "Running",
    className: "bg-indigo-900/40 text-indigo-400 border-indigo-700",
...
function formatNext(iso: string | null): string {
  ...
  if (isToday) return `Heute ${d.toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" })}`;
...
export default function JobsPage() {
  ...
  const displayedJobs =
    tab === "history"
      ? jobs.filter((j) => j.lastRunAt !== null)
      : jobs;
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Wiederkehrende Aufgaben für das Agent Network</p>
          </div>
          <button
            onClick={openNew}
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-500">
            ...
            <p className="text-lg font-medium text-gray-400">Keine Jobs gefunden</p>
            <p className="text-sm mt-1">Erstelle deinen ersten Job mit "New Job"</p>
          </div>
        ) : tab === "by-agent" ? (
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">Job Name</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">Beschreibung / Prompt</label>
                <textarea
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">Skill (optional)</label>
                <input
                  type="text"
                  value={form.skill}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-400 mb-1.5">Kadenz</label>
                  <select
                    value={form.schedule}
                ...
                <div>
                  <label className="block text-xs text-gray-400 mb-1.5">Uhrzeit</label>
                  <input
                    type="time"
                    value={form.time}
              ...
              <div className="flex items-center justify-between py-2">
                ...
                <button
                  ...
                  className={`relative w-10 h-5 rounded-full transition-colors ${
                    form.active ? "bg-indigo-600" : "bg-gray-700"
                  }`}
                >
                  <span
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 hover:border-gray-700 transition-colors">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {!job.active && (
                  <span className="text-xs text-gray-500 bg-gray-800 px-1.5 py-0.5 rounded">Inaktiv</span>
                )}
              </div>
          ...
          <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
            ...
            {job.skill && <span className="hidden md:block text-indigo-400/70">🔧 {job.skill}</span>}
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-800">
        <button
          ...
          disabled={running}
          className="flex items-center gap-1.5 text-xs text-indigo-400 hover:text-indigo-300 bg-indigo-900/20 hover:bg-indigo-900/40 border border-indigo-800/40 px-3 py-1.5 rounded-lg transition-all disabled:opacity-50"
        >
          {running ? "⏳ Running..." : "▶ Run Now"}
        </button>
        <button
          onClick={() => onEdit(job)}
          className="text-xs text-gray-400 hover:text-gray-200 bg-gray-800 hover:bg-gray-700 px-3 py-1.5 rounded-lg transition-colors"
        >
          ✏️ Edit
        </button>
        <button
          onClick={() => void onDelete(job.id)}
          className="text-xs text-red-500 hover:text-red-400 bg-red-900/10 hover:bg-red-900/20 px-3 py-1.5 rounded-lg transition-colors"
        >
          🗑️ Delete
        </button>
      </div>
